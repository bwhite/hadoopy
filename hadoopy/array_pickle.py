#!/usr/bin/env python
"""
This is a module used to efficiently encode a list of string encodings of array data.  The result is concatenated strings with the number of strings encoded as a base64 of a 32bit uint.  The list of string encodings will simply be concatenated, if you need base64 than perform it outside of this module.
...
def dumps2d64(vecs):
    """Takes in a list of equal length string encodings of array data.  It concatenates them all together and puts the length at the end in the form of a base64 encoded unsigned int."""
    return ''.join(vecs) + base64.b64encode(np.array(len(vecs), dtype=np.uint).tostring())
